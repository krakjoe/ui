...
void php_ui_entry_change_handler(uiEntry *u, void *_entry) {
	...
	if (entry->change.fci.size) {
		...
		if (php_ui_call(&entry->change.fci, &entry->change.fcc) != SUCCESS) {
			return;
		}
...
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(php_ui_entry_get_text_info, 0, 0, IS_STRING, 0)
#else
...
PHP_MINIT_FUNCTION(UI_Entry) 
{
	...
	INIT_NS_CLASS_ENTRY(ce, "UI\\Controls", "Entry", php_ui_entry_methods);
